...
public class YamlManager implements DbManager {
    ...
    public boolean deleteRecords(String playerName, int numberOfRecords) {
        ...
        // 如果记录列表为空或者指定删除数量大于记录数，返回 false
        if (records.isEmpty() || numberOfRecords <= 0) {
            return false;
        }

        // 使用 Stream API 处理删除操作
        List<String> updatedRecords = records.stream()
                .limit(records.size() - numberOfRecords) // 保留不需要删除的记录
                .collect(Collectors.toList());

        // 更新玩家记录
        playerConfig.set(path, updatedRecords);
        // 保存更新后的配置
        saveConfig(playerConfig, playerYaml);

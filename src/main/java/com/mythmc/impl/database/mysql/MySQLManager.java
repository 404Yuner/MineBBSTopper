...
public class MySQLManager implements DbManager {
     ...
     * 删除指定数量的记录的方法。
     *
     * @param playerName 删除的记录关联的玩家名称
     * @param numberOfRecords 删多少条
     * @return 删除是否成功
     */
    public boolean deleteRecords(String playerName, int numberOfRecords) {
        // 使用 JOIN 和 LIMIT 删除指定数量的记录
        String sql = "DELETE minebbstopper_data FROM minebbstopper_data " +
                "JOIN (SELECT rowid FROM minebbstopper_data WHERE player = ? LIMIT ?) AS to_delete " +
                "ON minebbstopper_data.rowid = to_delete.rowid";
        ...
        try (Connection connection = MySQLConnection.getConnection();
            ...
            return rowsAffected > 0; // 删除成功（删除了至少一行记录）
        } catch (SQLException e) {
            e.printStackTrace();
            return false; // 删除失败
        }

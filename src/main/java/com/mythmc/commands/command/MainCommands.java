    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length > 0) {
            switch (args[0].toLowerCase()) {
                case "reload":
                    return (new Reload()).handle(sender, plugin);
                case "open":
                    return (new Open()).handle(player, args);
                case "test":
                    return (new Test()).handle(player, args);
                case "url":
                    return (new URL()).handle(sender);
                case "claim":
                    return (new Claim()).handle(player, plugin);
                case "edit":
                    return (new Editor()).handle(sender, args);
                default:
